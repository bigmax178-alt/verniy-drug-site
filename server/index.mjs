#!/usr/bin/env node
// Сервер админки приюта «Верный друг».
//
// Делает три вещи:
//   1) даёт руководителю приюта удобно вести карточки животных;
//   2) принимает от посетителей сайта заявки на опекунство — вместе с согласием на
//      обработку персональных данных, и хранит их так, как требует 152-ФЗ;
//   3) отдаёт публичную часть данных сборщику сайта и запускает пересборку.
//
// Персональные данные (заявки, журнал согласий) остаются ТОЛЬКО здесь, на сервере
// в России, и никогда не попадают ни в git, ни в статическую сборку сайта.
//
// Запуск:  DATA_DIR=./data-runtime node server/index.mjs
...
import { normalizeAnimal, toPublicAnimal } from './lib/animal.mjs';
import { readJson, readForm, readBody, parseCookies, setCookie, send, sendHtml, sendJson, redirect, token, createRateLimiter, clientIp, MAX_UPLOAD } from './lib/http.mjs';
...
// Домены сайта, которым разрешено отправлять форму заявки.
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || 'http://localhost:4321')
  .split(',')
  .map((s) => s.trim().replace(/\/$/, ''))
  .filter(Boolean);
const COOKIE_SECURE = process.env.COOKIE_SECURE !== '0'; // выключать только для локальной отладки по http
...
async function route(req, res, url) {
  ...
  if (p === '/admin/animal/new' && method === 'GET') {
    return sendHtml(res, 200, views.animalFormPage({ session, animal: null }));
  }
  ...
  if (editMatch && method === 'GET') {
    ...
    return sendHtml(res, 200, views.animalFormPage({ session, animal }));
  }
  ...
  if (editMatch && method === 'POST') {
    ...
    if (!result.ok) {
      return sendHtml(res, 400, views.animalFormPage({ session, animal: { ...(existing || {}), ...input, id: editMatch[1] }, errors: result.errors }));
    }
    await store.upsertAnimal(result.animal);
    await store.audit('animal_saved', { by: session.login, id: result.animal.id, name: result.animal.name });
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`Формы принимаются с: ${ALLOWED_ORIGINS.join(', ') || '(не задано)'}`);
